...
//    auto temp_snake_board =  snake_.getPosition();
        ...
        void Userpad::Draw() {
            for (size_t row = 0; row < num_pixels_per_side_; ++row) {
                for (size_t col = 0; col < num_pixels_per_side_; ++col) {
                    ...
                    vec2 pixel_bottom_right =
                            pixel_top_left + vec2(pixel_side_length_, pixel_side_length_);
                    ci::gl::color(ci::Color::gray(0.9f));
        ...
        void Userpad::DrawFood() {
            int x = food.first;
            int y = food.second;
            if (food.first == -1) {
                x = rand() % 28;
                y = rand() % 28;
                while (snake_vec_[x][y]) {
                    x = rand() % 28;
                    y = rand() % 28;
                }
            }
        ...
        const std::pair<int, int> &Userpad::getFood() const {
            return food;
        }

        void Userpad::setFood(const std::pair<int, int> &food) {
            Userpad::food = food;
        }
